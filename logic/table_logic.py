...
def create():
    ...
    a1=Database.get()
    mycursor.execute("CREATE DATABASE %s"%a1)
    reset_database()
def reset_database():
    Database.set("")
    Database_delete.set("")
    Database_connect.set("")
...
def delete_database():
    mydb = mysql.connector.connect(host="localhost", user="root",password="password")
    mycursor = mydb.cursor()
    b1=Database_delete.get()
    mycursor.execute("DROP DATABASE %s" % b1)
    reset_database()
    show_database()

def connection():
    c1 = Database_connect.get()
    messagebox.showinfo("Sucess","Hogya")
    ...
    def update():
        mydb = mysql.connector.connect(host="localhost", user="root", database=c1, password="password")
        mycursor = mydb.cursor()
        mycursor.execute(
            "Update First8 SET Agency=%s,Medicine=%s,Batch=%s,Rate=%s,Quantity=%s,Expiry=%s,Date=%s,Billno=%s WHERE Id=%s",
            (Agency_name.get(), Medicine_name.get(), Batch_name.get(), rate_name.get(), quantity_name.get(),
             Exp_name.get(), Date_name.get(), Billno_name.get(), Id.get()))
        mydb.commit()
        show()
        reset()
        mydb.close()
    ...
    def submit():
        if (
                Agency_name.get() != "" and Medicine_name.get() != "" and Exp_name.get() != "" and Date_name.get() != "" and Billno_name.get() != " "):
            mydb = mysql.connector.connect(host="localhost", user="root", database=c1, password="password")
            mycursor = mydb.cursor()
            mycursor.execute(
                "CREATE TABLE IF NOT EXISTS First8 (Id int(16) auto_increment,Agency VARCHAR(255), Medicine VARCHAR(255),Batch VARCHAR(255),Rate VARCHAR(255),Quantity VARCHAR(255),Expiry DATE,DATE DATE,Billno Varchar(255),PRIMARY KEY (Id))")

            sql = "INSERT INTO First8 (Agency,Medicine,Batch,Rate,Quantity,Expiry,DATE,Billno) VALUES (%s, %s,%s,%s,%s,%s,%s,%s)"
            val = (Agency_name.get(), Medicine_name.get(), Batch_name.get(), rate_name.get(), quantity_name.get(),
                   Exp_name.get(), Date_name.get(), Billno_name.get())
    ...
    def Take_backup():
        mydb = mysql.connector.connect(host="localhost", user="root", database=c1, password="password")
        mycursor = mydb.cursor()
        mycursor.execute("SELECT * From First8")
        result=mycursor.fetchall()
        save_path = (r"E:\project")
        completeName = os.path.join(save_path,"Backup.csv")
        with open(completeName, 'a+', newline="") as csvfile:
            csvwriter = csv.writer(csvfile)
            for x in result:
    ...
    Backup_btn = Button(f3, text="Take Backup", width=10, command=Take_backup)
    Backup_btn.grid(row=0, column=5)
    # -----------------frame--------------------------------------
    ...
    main_tabel = ttk.Treeview(tabel_frame, columns=(
    "Agency", "Medicine", "Billno", "Expiry", "Batch", "Rate", "Quantity", "Id", "Date"), xscrollcommand=scroll_x.set,
...
Database=StringVar()
Database_delete=StringVar()
Database_connect=StringVar()

label=Label(root,text="Create Databse",font=("Arial",20),fg="black")
label.grid(row=0,column=0,pady=10,padx=10,sticky="w")
Create_name=Entry(root,textvariable=Database,font=("Arial",15),bg="White",fg="Black",relief="flat",bd=0)
Create_name.grid(row=0,column=1,pady=12,padx=10)
base=Button(root,text="Create",command=create)
base.grid(row=0,column=2)

label2=Label(root,text="Delete Databse",font=("Arial",20),fg="black")
label2.grid(row=1,column=0,pady=10,padx=10,sticky="w")
delete_name=Entry(root,textvariable=Database_delete,font=("Arial",15),bg="White",fg="Black",relief="flat",bd=0)
delete_name.grid(row=1,column=1,pady=12,padx=10)
delete=Button(root,text="Delete",command=delete_database)
delete.grid(row=1,column=2)

label3=Label(root,text="Connect to Database",font=("Arial",20))
label3.grid(row=2,column=0,pady=10,padx=10,sticky="w")
t_name=Entry(root,textvariable=Database_connect,font=("Arial",15),bg="White",fg="Black",relief="flat",bd=0)
t_name.grid(row=2,column=1,pady=12,padx=10)
table_name=Button(root,text="get_connect",command=connection)
table_name.grid(row=2,column=2,pady=12,padx=10)

show_all=Button(root,text="show all databases",command=show_database)
show_all.grid(row=2,column=3)
